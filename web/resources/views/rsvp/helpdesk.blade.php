		...
		$.ajax({
			...
			success:function(result) {
				...
				for (var i = 0; i < result.length; i++) {
					html += '<tr><td>'+result[i].name+'</td><td>'+result[i].email+'</td><td>'+result[i].phone+'</td><td><span onclick="popitup(\'{{url('rsvp/print')}}/'+result[i].reg_number+'/Helpdesk\');" class="button primary">Print Seat Number</span></td><td><span onclick="popitup(\'{{url('rsvp/print_qr')}}/'+result[i].reg_number+'\');" class="button primary">Print QR Code</span></td></tr>';
				}
				$("#content").html(html);
			}
		})
